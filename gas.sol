...
contract T8 {
    ...
    uint256 public initialGas = gasleft();
    ...
    function calcYearOfBirth(uint256 _age, uint256 _current_year) public returns(uint256, uint256) {
        (bool check, uint256 year_of_birth) = SafeMath.trySub(_current_year, _age);
        require(check, "You entered wrong current year or year of birth");
        uint256 n;
        uint256 gas;
        gas = gasleft();
        //n = initialGas - gas;
        return (year_of_birth, gasleft());
    }
